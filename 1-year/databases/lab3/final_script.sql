...
-- Trigger to check if fuel_left is more than its possible and changing it to max value if its neccessary
...
-- Auxiliary function: returns distance between two paoints
...
-- Trigger function that changes spaceships params (fuel_left, is_crashed) when it moves somewhere
...
-- I want to see a table that shows how much fuel spaceship will use to move for some distance <3
